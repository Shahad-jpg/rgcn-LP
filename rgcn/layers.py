...
class DistMult(Module):
    ...
    class RelationalGraphConvolutionLP(Module):
        ...
        def __init__(self,
            ...
            num_bases = decomposition[
                'num_bases'] if decomposition is not None and 'num_bases' in decomposition else None
        ...
        def initialise_weights(self):
            ...
            if self.weight_decomp == 'block':
                schlichtkrull_normal_(self.blocks,
                                      shape=[(self.num_relations - 1) // 2, self.in_features // self.num_blocks],
                                      gain=gain)
                # Checkpoint 3
                ...
                schlichtkrull_normal_(self.blocks_self,
                                      shape=[(self.num_relations - 1) // 2, self.in_features // self.num_blocks],
                                      gain=gain)
                # Checkpoint 4
        ...
        def forward(self, triples, features=None):
            ...
            if self.in_features is None:
                ...
                if self.weight_decomp == 'block':
                    ...
                    assert fw.shape == (
                    r, n, num_blocks, output_block_size), f"{fw.shape}, {(r, n, num_blocks, output_block_size)}"
                    fw = fw.view(r, n, out_dim)
                    self_fw = torch.einsum('ni, io -> no', features, self.blocks_self)[None, :, :]
                    if self.training and self.edge_dropout["self_loop_type"] == 'schlichtkrull-dropout':
                        self_fw = nn.functional.dropout(self_fw, p=self.edge_dropout["self_loop"], training=True,
                                                        inplace=False)
...
#def RelationalGraphConvolutionLP():
#    return None